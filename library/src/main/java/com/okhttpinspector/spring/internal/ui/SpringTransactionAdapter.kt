...
import com.okhttpinspector.spring.internal.ui.SpringTransactionListFragment.OnListFragmentInteractionListener

internal class SpringTransactionAdapter(private val context: Context, private val listener: OnListFragmentInteractionListener?) : RecyclerView.Adapter<SpringTransactionAdapter.ViewHolder>() {
    private val cursorAdapter: CursorAdapter

    private val colorDefault: Int = ContextCompat.getColor(context, R.color.spring_status_default)
    private val colorRequested: Int = ContextCompat.getColor(context, R.color.spring_status_requested)
    private val colorError: Int = ContextCompat.getColor(context, R.color.spring_status_error)
    private val color500: Int = ContextCompat.getColor(context, R.color.spring_status_500)
    private val color400: Int = ContextCompat.getColor(context, R.color.spring_status_400)
    private val color300: Int = ContextCompat.getColor(context, R.color.spring_status_300)

    init {

        cursorAdapter = object : CursorAdapter(this@SpringTransactionAdapter.context, null, CursorAdapter.FLAG_REGISTER_CONTENT_OBSERVER) {
            override fun newView(context: Context, cursor: Cursor, parent: ViewGroup): View {
                val itemView = LayoutInflater.from(parent.context).inflate(R.layout.spring_list_item_transaction, parent, false)
            ...
            override fun bindView(view: View, context: Context, cursor: Cursor) {
                ...
                holder.view.setOnClickListener {
                    if (null != this@SpringTransactionAdapter.listener) {
                        this@SpringTransactionAdapter.listener.onListFragmentInteraction(holder.transaction!!)
                    }
                }
            }
